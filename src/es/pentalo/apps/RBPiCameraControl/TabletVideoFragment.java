...
public class TabletVideoFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		myView = inflater.inflate(R.layout.tablet_video, container, false);
		
		RBPreferenceFragment videoFragment = new RBPreferenceFragment();	
		videoFragment.setFeature(RBPFeatures.VIDEO);
		getFragmentManager().beginTransaction().replace(R.id.llPreference2, videoFragment).commit();
		getFragmentManager().beginTransaction().replace(R.id.llVideo, new StreamingFragment()).commit();
